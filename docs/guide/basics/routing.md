...
graph TD
    ...
    D["优点"] -->|适合小型网络<br/>延迟最小| E["[v]"]
    F["缺点"] -->|扩展性差<br/>无故障自动恢复<br/>容易配置错误| G["[x]"]
```
...
graph LR
    ...
    C -->|"哎呀，我没配置<br/>到 10.2.1.50 的路由<br/>丢弃包！"| D["[x] 包丢失"]
...
好处：
  [v] 充分利用多条链路
  [v] 容量翻倍（从 10Mbps 增加到 30Mbps）
  [v] 故障保护（一条链路故障，只影响 33% 的流量）

坏处：
  [x] 基于五元组（源 IP、目标 IP、源端口、目标端口、协议）的哈希
  [x] 可能导致不同包乱序（TCP 可以处理，但降低效率）
  [x] 配置复杂，需要精心规划
```
...
状态：
  [x] 部署缓慢：全球仅 30% 的 ISP 支持 RPKI
  [v] 新兴协议：大型 ISP（Google、AWS、Cloudflare）在积极推动
