...
> <Icon name="clipboard-list" color="cyan" /> **前置知识**：[TLS 1.3详解](/guide/basics/tls)、[网络安全基础](/guide/attacks/encryption)
> ⏱ **阅读时间**：约18分钟
...
```mermaid
graph TD
    RC["[lock] Root CA<br/><small>根证书颁发机构</small><br/><small>自签名 · 离线存储</small>"]
    IC1["[gov] Intermediate CA 1<br/><small>中间证书颁发机构</small><br/><small>TLS 证书签发</small>"]
    IC2["[gov] Intermediate CA 2<br/><small>中间证书颁发机构</small><small>代码签名专用</small>"]
    EE1["[file] End-Entity Cert<br/><small>api.example.com</small>"]
    EE2["[file] End-Entity Cert<br/><small>www.example.com</small>"]
    EE3["[file] Code Signing Cert<br/><small>release@example.com</small>"]
...
sequenceDiagram
    ...
    C->>C: 2. 在证书链中找到 Intermediate CA 证书<br/>用其公钥验证 End-Entity 签名 [v]
    C->>C: 3. Intermediate CA 的 Issuer = Root CA DN
    C->>OS: 4. 查询本地信任库<br/>是否包含该 Root CA？
    OS->>C: 返回 Root CA 证书（受信任）
    C->>C: 5. 用 Root CA 公钥验证<br/>Intermediate CA 签名 [v]
    C->>C: 6. 检查有效期、吊销状态（OCSP/CRL）
    C->>C: 7. 检查 SAN 是否匹配目标域名 [v]
...
graph LR
    ...
    MDM["[phone] MDM（设备管理）<br/>分发企业根证书"] -.->|"推送信任"| CorpRoot
...
```mermaid
flowchart LR
    A["[key] 生成密钥对\n私钥留存本地"] --> B["[note] 生成 CSR\n证书签名请求"]
    B --> C{"CA 审核\nDV/OV/EV"}
    C -->|"验证通过"| D["[scroll] CA 签发证书\n返回 PEM 文件"]
    C -->|"验证失败"| B
    D --> E["[run] 部署证书\n服务器/负载均衡"]
    E --> F["[eye] 监控证书状态\n到期预警 · 吊销检测"]
    F -->|"临近到期（30天）"| G["[sync] 自动续期\nACME 协议"]
    G --> E
    F -->|"私钥泄露 / 信息变更"| H["[x] 申请吊销\nCRL + OCSP 更新"]
    H --> A
...
sequenceDiagram
    ...
    C->>C: 验证 OCSP Response 签名 [v]<br/>无需额外 DNS/OCSP 请求
