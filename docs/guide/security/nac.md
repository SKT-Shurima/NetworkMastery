...
> 📋 **前置知识**：[零信任安全](/guide/security/zero-trust)、[Wi-Fi安全](/guide/wireless/wifi-security)
> ⏱️ **阅读时间**：约18分钟
...
基础层（必须）：
  ✓ 所有接入端口部署 802.1X（或 MAB 兜底）
  ✓ 访客网络与内网物理隔离
  ✓ 认证日志保留 90 天以上

进阶层（推荐）：
  ✓ Posture 合规检查覆盖所有托管设备
  ✓ CoA 自动响应威胁事件
  ✓ BYOD 证书自动颁发
  ✓ MDM 集成实现设备状态实时感知
...
高级层（成熟企业）：
  ✓ SGT 微分段代替大 VLAN
  ✓ ISE pxGrid 与 SIEM/SOAR 联动
  ✓ 零信任持续验证（每 24h 重新评估 Posture）
  ✓ 网络访问记录与行为基线分析
```
