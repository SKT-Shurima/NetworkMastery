...
> 📋 **前置知识**：[云网融合基础](/guide/cloud/hybrid-networking)、[IPSec VPN](/guide/security/ipsec)
> ⏱️ **阅读时间**：约20分钟
...
graph TB
    subgraph "AWS全球基础设施层次"
        A["🌍 Global<br/>全球骨干网络"]
        B["🗺️ Region（区域）<br/>ap-northeast-1 东京<br/>us-east-1 弗吉尼亚"]
        C["🏢 Availability Zone（可用区）<br/>AZ-a / AZ-b / AZ-c"]
        D["🔒 VPC（虚拟私有云）<br/>10.0.0.0/16"]
        E["📦 Subnet（子网）<br/>10.0.1.0/24 公有<br/>10.0.10.0/24 私有"]
        F["💻 ENI（弹性网络接口）<br/>EC2 / Lambda / RDS"]
    end
...
graph TB
    subgraph VPC["VPC: 10.1.0.0/16"]
        subgraph AZ_A["可用区 AZ-a"]
            PubA["公有子网 (Public Subnet)<br/>10.1.1.0/24<br/>🌐 ALB / NAT GW / Bastion"]
            AppA["应用子网 (App Subnet)<br/>10.1.11.0/24<br/>⚙️ EC2 / ECS / Lambda"]
            DbA["数据子网 (DB Subnet)<br/>10.1.21.0/24<br/>🗄️ RDS / ElastiCache"]
        end
...
```mermaid
graph TB
    subgraph TGW_Hub["☁️ Transit Gateway (TGW) - 枢纽"]
        TGW["🔄 Transit Gateway<br/>tgw-0abc1234"]
    ...
    subgraph "混合云连接"
        ...
        OnPrem["🏢 企业数据中心<br/>172.16.0.0/12"]
    end
...
graph LR
    subgraph "企业本地"
        DC["🏢 企业数据中心<br/>On-Premises Router"]
    end

    subgraph "运营商/托管设施"
        Colocation["🏭 互联站（DX Location）<br/>上海核心/北京丰台"]
...
graph TB
    subgraph "企业本地（On-Premises）"
        DC_Router["🏢 核心路由器<br/>BGP AS 65000"]
        DC_FW["防火墙/UTM"]
