...
> 📋 **前置知识**：[AWS云网络](/guide/cloud/aws-networking)、[Azure云网络](/guide/cloud/azure-networking)、[SD-WAN概念](/guide/sdwan/concepts)
> ⏱️ **阅读时间**：约18分钟
...
```mermaid
graph TB
    subgraph Users["👥 终端用户与分支机构"]
        Mobile[移动端用户]
        Branch[全国分支网点]
        Corp[企业办公网络]
    end

    subgraph GlobalLB["🌐 全局流量调度层"]
        GTM[Cloudflare Global<br/>Traffic Manager<br/>健康检查+地理调度]
    end

    subgraph AWS_Prod["☁️ AWS 上海/北京（主力生产）"]
    ...
    subgraph Azure_DR["☁️ Azure 中国东部（双活容灾）"]
        FD[Azure Front Door]
