# 网络故障排查实战：tcpdump、traceroute、ping 深度用法

## 导言

网络出问题了。员工说："网络很卡"。IT 负责人看着 Dashboard，数据好像都正常。这是网络管理员最常遇到的困境。

这一章不是讲故障排查的流程图，而是传授实战中的==排查技能==和==思维方式==。我们会通过真实的故障案例，展示如何用 ping、traceroute、tcpdump 等工具快速定位问题。
