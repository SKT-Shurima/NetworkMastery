...
```mermaid
graph TB
    subgraph User["[user] 用户与设备"]
        Remote["[home] 远程员工<br/>家庭网络<br/>不可信环境"]
        Branch["[corp] 分支行员工<br/>办公网络<br/>部分可信"]
        Mobile["[phone] 移动设备<br/>CEO、高管<br/>极限信任"]
    end
    
    subgraph Auth["[lock] 身份与信任评估层"]
    ...
    subgraph GW["[net] SD-WAN 边界网关"]
        SGW["安全网关<br/>• 流量加密<br/>• 零信任检查<br/>• 微分段实施<br/>• DLP 触发"]
    end
    
    subgraph Decision["[law] 访问决策与执行"]
        Decision1["[pin] 策略 1: 财务员工<br/>[v] 身份: OK<br/>[v] 设备: 可信<br/>[v] 地点: 分支行<br/>→ 发放财务系统令牌<br/>→ 有效期: 4 小时"]
        Decision2["[pin] 策略 2: IT 远程员工<br/>[v] 身份: OK<br/>[!] 设备: 中等信任<br/>[!] 地点: 家庭<br/>→ 发放限制令牌<br/>→ 需要 VPN 二次认证<br/>→ 有效期: 2 小时"]
        Decision3["[pin] 策略 3: 高管移动<br/>[v] 身份: OK<br/>[!] 设备: 无管理<br/>[v] 地点: GPS 已验证<br/>→ 只读权限<br/>→ 禁止下载<br/>→ 有效期: 1 小时"]
    end
    
    subgraph CoreSys["[work] 核心应用系统"]
        FinSys["金融交易系统<br/>隔离网段<br/>微分段: FS-001"]
        ERPSys["ERP 系统<br/>隔离网段<br/>微分段: ERP-001"]
        OtherSys["其他应用<br/>多个隔离网段<br/>基于角色隔离"]
    end
    
    subgraph Monitor["[chart] 监控与审计"]
    ...
    Decision1 -->|[v] 高权限令牌| FinSys
    Decision1 -->|[v] 普通权限| ERPSys
    
    Decision2 -->|[!] 限制令牌| ERPSys
    Decision2 -->|[x] 禁止| FinSys
    
    Decision3 -->|[doc] 只读令牌| FinSys
    Decision3 -->|[doc] 只读令牌| ERPSys
...
graph TB
    subgraph Network["网络分段"]
        Seg1["[lock] 财务部网段<br/>VLAN: 1001<br/>只允许到: 财务系统<br/>用户: 财务部员工"]
        Seg2["[lock] 交易部网段<br/>VLAN: 1002<br/>只允许到: 交易系统<br/>用户: 交易部员工"]
        Seg3["[lock] IT 管理网段<br/>VLAN: 1003<br/>允许到: 所有系统<br/>用户: IT 管理员"]
        Seg4["[lock] 客户服务网段<br/>VLAN: 1004<br/>只允许到: 客户系统<br/>用户: 一线员工"]
        Seg5["[phone] 访客网段<br/>VLAN: 1005<br/>只允许到: 互联网<br/>用户: 临时访客"]
    end
    
    subgraph Rules["防火墙规则"]
        R1["[v] Seg1 → 财务系统"]
        R2["[x] Seg1 → 交易系统<br/>(跨段被阻止)"]
        R3["[v] Seg3 → * (管理权限)"]
        R4["[x] Seg4 → Seg1<br/>(横向移动被防止)"]
    end
...
sequenceDiagram
    ...
    D->>Trust: 上报设备状态
    Trust->>Trust: 检查 MDM 状态 [v]
    Trust->>Trust: 检查防病毒版本 [v]
    Trust->>Trust: 检查补丁状态 [v]
    Trust->>Trust: 综合评分: 95/100 (高信任)
    
    Trust->>Gate: 转发评分 + 风险评估
    Gate->>Gate: 查询策略库
    Gate->>Gate: 李四 = 财务员工
    Gate->>Gate: 地点 = 分支行(GPS验证) [v]
