...
```mermaid
graph TB
    subgraph HQ["[corp] 总部（苏州）"]
        Controller["[chart] SD-WAN Controller<br/>控制器 + 分析引擎<br/><br/>核心功能：<br/>• 策略下发<br/>• 流量分析<br/>• 故障检测<br/>• 成本优化"]
        Monitor["[up] 监控与分析<br/>实时仪表板<br/>• 链路质量<br/>• 成本追踪<br/>• 告警管理"]
    end
    
    subgraph Factory1["[factory] 大工厂 1 (华东)"]
    ...
    subgraph Internet["[cloud] 互联网出口"]
        Router["智能路由选择<br/>• 公网（ISP）<br/>• 运营商 MPLS<br/>• 云服务接入<br/>• QoS 管控"]
    end
    
    subgraph Policy["[cfg] 智能策略引擎"]
        P1["[pin] 规则 1: ERP 流量<br/>优先级：最高<br/>路径：MPLS > 专网 > 宽带<br/>SLA：延迟&lt;100ms, 丢包&lt;0.1%"]
        P2["[pin] 规则 2: WEB 应用<br/>优先级：中等<br/>路径：智能漂移<br/>SLA：自适应"]
        P3["[pin] 规则 3: 日志备份<br/>优先级：低<br/>时间：非工作时间<br/>成本优化"]
    end
    
    Controller -->|[sync] 策略下发| GW1
    Controller -->|[sync] 策略下发| GW2
    Controller -->|[sync] 策略下发| GW3
    
    Monitor -->|[chart] 实时监控| Controller
    
    GW1 <-->|[lock] IPSec 隧道| GW2
    GW2 <-->|[lock] IPSec 隧道| GW3
    GW1 <-->|[radio] IPSec 隧道| GW3
