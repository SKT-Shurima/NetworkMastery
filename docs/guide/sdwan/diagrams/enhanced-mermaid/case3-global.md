...
graph TB
    subgraph Scenario["场景: 全球代码库同步"]
        Dev["👨‍💻 开发场景<br/>中国深圳开发者<br/>提交代码到全球 repo<br/>CI/CD 流程触发"]
    end
    
    subgraph TrafficRoute["智能路由与合规"]
        Route1["代码提交:<br/>深圳 → 新加坡 Hub<br/>延迟: 50ms<br/>• 在国内完成初步处理<br/>• 数据不出国<br/>✓ 合规"]
        
        Route2["CI 编译:<br/>新加坡 → 美国<br/>延迟: 120ms<br/>• 利用美国计算能力<br/>• 数据跨境转移<br/>⚠️ 经过审查"]
        
        Route3["测试与部署:<br/>美国 → 欧洲<br/>延迟: 160ms<br/>• 欧洲测试环境<br/>• GDPR 数据隔离<br/>✓ 符合要求"]
        
        Route4["反馈:<br/>欧洲 → 深圳<br/>延迟: 280ms<br/>• 优化后的路由<br/>• 数据不触及敏感区<br/>✓ 安全"]
    end
    
    subgraph Decision["决策因素"]
        Factor1["📍 地理位置<br/>数据主权法规<br/>• 中国数据内地<br/>• 欧洲数据欧盟<br/>• 美国可跨域"]
        
        Factor2["🚀 性能需求<br/>• 开发: 低延迟优先<br/>• CI: 计算资源优先<br/>• 存储: 冗余备份"]
        
        Factor3["💰 成本控制<br/>• 跨国带宽最贵<br/>• 优化路由降成本<br/>• 利用 ISP 本地化"]
    end
    
    subgraph Outcome["最终效果"]
        O1["⏱️ 延迟改进<br/>从 800ms 降至 150ms<br/>整体周期减少 70%"]
        
        O2["💵 成本控制<br/>跨国带宽成本 -45%<br/>利用当地便宜链路"]
        
        O3["✅ 法规合规<br/>数据主权完全保护<br/>审计链完整"]
    end
