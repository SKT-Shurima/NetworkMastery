...
> 📋 **前置知识**：[Ansible网络自动化](/guide/automation/ansible-network)、[IaC网络管理](/guide/automation/iac-networking)
> ⏱️ **阅读时间**：约18分钟
...
graph TD
    ...
    PROD_MONITOR -->|指标异常| ROLLBACK["🔄 自动回滚"]
...
def run_connectivity_matrix(env: str) -> bool:
    ...
    print(f"✅ 所有连通性测试通过 ({len(matrix['tests'])} 个测试点)")
    return True
    ...
    for device_name, routes in expected.items():
        ...
        for route in routes:
            if route['prefix'] not in routing_table:
                print(f"❌ {device_name} 缺少预期路由: {route['prefix']}")
                return False
...
flowchart TD
    ...
    AUTO_DECISION -->|"错误率 > 5%\n或完全中断"| AUTO_ROLLBACK["🔄 自动回滚\n立即执行\n无需人工干预"]
    
    AUTO_DECISION -->|"1% < 错误率 ≤ 5%\n降级但未中断"| HUMAN{"工程师研判\n5分钟内决策"}
    
    HUMAN -->|确认回滚| MANUAL_ROLLBACK["🔄 手动触发回滚"]
    HUMAN -->|继续观察| MONITOR
