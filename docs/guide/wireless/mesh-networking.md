...
> 📋 **前置知识**：[Wi-Fi 6/7标准](/guide/wireless/wifi-standards)、[网络拓扑设计](/guide/architecture/topology)
> ⏱️ **阅读时间**：约16分钟
...
| 角色 | 英文名称 | 功能描述 | 有线上联 |
|------|----------|----------|----------|
| **根节点** | Root AP / MAP（Mesh Access Point）with Ethernet Backhaul | 连接有线网络，是Mesh域的出口 | ✅ 必须 |
| **Mesh节点** | MAP（Mesh Access Point）| 通过无线回程中继，覆盖延伸区域 | ❌ 可选 |
...
sequenceDiagram
    ...
    participant AP_OLD as 🔵 当前关联AP (AP-Old)
    participant AP_NEW as 🟢 目标AP (AP-New)
    participant DS as 🏛️ 分布式系统/控制器 (DS)
    ...
    Note over STA,AP_NEW: ✅ 总耗时目标 < 50ms，语音/视频不中断
```
